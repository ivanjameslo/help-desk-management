...
export async function updateUserAccess(
  ...
  const { role: nextRole, isActive: nextIsActive } = validatedFields.data;
  ...
  if (targetUser.id === currentAdmin.id && (nextRole !== UserRole.ADMIN || !nextIsActive)) {
    return {
  ...
  if (removeActiveAdministrator) {
    const otherActiveAdministratorCount = await prisma.user.count({
      where: {
        id: {
          not: targetUser.id,
        },
    ...
    if (otherActiveAdministratorCount === 0) {
      return {
        message:
          "This account is the system's last active administrator and cannot be demoted or deactivated.",
        success: false,
      };
    }
  }

  if (targetUser.role === nextRole && targetUser.isActive === nextIsActive) {
    return {
