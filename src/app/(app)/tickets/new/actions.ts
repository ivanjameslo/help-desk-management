...
export async function createTicket(
  ...
  const { subject, description, categoryId, priority } = validatedFields.data;
  ...
  const requester = await requireRole([UserRole.REQUESTER]);
  ...
  try {
    await prisma.$transaction(async (transaction) => {
      const ticket = await transaction.ticket.create({
        data: {
          ticketNumber: `HD-${randomUUID().slice(0, 8).toUpperCase()}`,
          subject,
          description,
          priority,
