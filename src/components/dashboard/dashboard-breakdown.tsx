...
export function DashboardBreakdown({ title, items, total }: DashboardBreakdownProps) {
  return (
    <section className="min-w-0 rounded-xl border bg-white p-4 shadow-sm sm:p-6">
      <h2 className="text-base font-semibold text-gray-900 sm:text-lg 2xl:text-xl">{title}</h2>
            ...
            return (
              <div key={item.label}>
                <div className="flex items-center justify-between gap-3 sm:gap-4">
                  <p className="min-w-0 text-xs font-medium wrap-break-word text-gray-700 sm:text-sm 2xl:text-base">
                    {item.label}
                  </p>
